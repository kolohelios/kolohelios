{
  description = "cascadiajs2026-notes";
        ...
        { pkgs, ... }:
        {
          default = pkgs.mkShell {
            packages = [
              ...
              pkgs.pkg-config
              pkgs.openssl
              # Tailwind CSS compiler — invoked by `build-site` after
              # it renders the talk notes to dist/. nixpkgs ships the
              # standalone Go binary; no Node toolchain required.
              pkgs.tailwindcss
              # `cue` exports `data/talks.cue` to JSON that `build-site`
              # deserializes for talk metadata. Same tool the shaka
              # schema/audit checks use, exposed here for the build path.
              pkgs.cue
            ]
