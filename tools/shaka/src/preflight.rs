...
fn just_validate(project_dir: &Path) -> CheckResult {
    ...
    // project's tools on PATH (e.g. tofu for infra/devbox, cargo for
    // tools/shaka). Without this, the validate recipe inherits whatever
    // shell preflight was invoked from, which won't have project-specific
    // dependencies.
    run_command(
        Command::new("nix")
...
fn run_command(cmd: &mut Command) -> CheckResult {
    ...
    // develop closures), capturing output keeps the user in the dark
    // for minutes. Captured-output replay-on-failure isn't worth the
    // silence — the user sees the failure inline.
