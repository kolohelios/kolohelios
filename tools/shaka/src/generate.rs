...
const ROOT_TEMPLATE: &str = r#"# Run every CI check locally
validate:
    shaka preflight
...
pub fn run(check: bool) {
    ...
    let mut items = vec![(PathBuf::from("justfile"), render_root())];
...
fn write_all(items: &[(PathBuf, String)]) {
    ...
    println!("\n{GREEN}{BOLD}generate:{RESET} wrote {} files", items.len());
}
...
fn check_drift(items: &[(PathBuf, String)]) {
    ...
    if !drift.is_empty() {
        eprintln!(
            "{RED}{BOLD}generate --check failed{RESET}: {} file(s) drifted",
            drift.len()
        );
        eprintln!("{YELLOW}Run `shaka generate` and commit the result.{RESET}");
        eprintln!(
            "{DIM}drifted: {}{RESET}",
    ...
    println!("{GREEN}{BOLD}generate --check passed{RESET}");
}
...
mod tests {
    ...
    #[test]
    fn render_root_uses_root_template() {
        let out = render_root();
        assert!(out.contains("validate:"));
        assert!(out.contains("shaka preflight"));
    }
